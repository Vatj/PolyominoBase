...
def AddNewDataToFile(classifier,data_in,needle,mu):
    with open('/rscratch/asl47/Processed/Dynamic/Evolution_Solution_Times3_Mu{}_Needle{}.txt'.format(mu,needle), "a") as data_file:
...
def AddBulkDataToFile(As,Os,needle,mu):    
    for (A,O) in zip(As,Os):
        data=SolutionCDF(needle,A,mu,O,250)
        AddNewDataToFile('A{}O{}'.format(A,O),data,needle,mu)
        
def LoadExistingData(needle,mu):
    data_dict={key:[int(v) for v in values.split()] for (key,values) in [line.rstrip('\n').split(':') for line in open('/rscratch/asl47/Processed/Dynamic/Evolution_Solution_Times2_Mu{}_Needle{}.txt'.format(mu,needle))]}
    return data_dict
...
def SolutionCDF(needle_length=30,A=2,mu=4,O=3,runs=1):
    ...
    for r in xrange(runs):
        ...
        fitness_import=np.genfromtxt('/scratch/asl47/Data_Runs/Dynamic_3/A{}Mu{}O{}/{}_Fitness.txt'.format(A,mu,O,subfile_name))
