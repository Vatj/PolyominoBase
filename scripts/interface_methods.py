...
def RandomHamming(L=16):
     ...
     b=binom(L,0.5)
     b2=binom(L/2,0.5)
     plt.plot(np.linspace(0,1,L+1),[50000*(4*7)*b.pmf(i) for i in range(L+1)],'kx',markeredgewidth=1)
     plt.plot(np.linspace(0,1,9),[50000*(4*7)*b.pmf(i)+50000*8*b2.pmf(j) for i,j in zip(range(0,L+1,2),range(0,L/2+1))],'m+',markeredgewidth=1)
     hist,bins=np.histogram(bs,bins=np.linspace(0,L+1,L+2))
     plt.plot(bins[:-1]/L,hist)
     plt.plot(np.linspace(0,1,L+1),[N*b.pmf(i) for i in range(L+1)],'ro',markeredgewidth=1)
     plt.yscale('log')
